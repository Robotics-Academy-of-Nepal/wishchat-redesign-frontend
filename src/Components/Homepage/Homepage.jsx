...
export default function Homepage() {
  ...
  const text = {
    english: {
      ...
      botResponse: "Of course! I've found the latest updates from May 2025. Would you like me to summarize them for you?",
      inputPlaceholder: "Type your message...",
      getStarted: "Get Started"
    },
    nepali: {
      greeting: "नमस्ते! म तपाईंलाई कसरी सहयोग गर्न सक्छु?",
      userQuestion: "के तपाईं मलाई नवीनतम उत्पादन अपडेटहरू फेला पार्न मद्दत गर्न सक्नुहुन्छ?",
      botResponse: "पक्कै! मैले मे २०२५ को नवीनतम अपडेटहरू फेला पारेको छु। के तपाईं चाहनुहुन्छ कि म तिनीहरूको सारांश प्रस्तुत गरूँ?",
  ...
  return (
    <div className="flex flex-col min-h-screen overflow-hidden bg-gradient-to-b from-indigo-50 to-white">
      ...
      <div className="m-4 md:m-6 lg:m-8 rounded-3xl flex flex-col gap-6 pb-16 bg-gradient-to-br from-white via-indigo-100 to-indigo-300 shadow-xl overflow-hidden relative">
        ...
        <div className={`absolute top-0 right-0 w-64 h-64 ${chatDarkMode ? 'bg-indigo-600' : 'bg-indigo-200'} rounded-full opacity-20 transform translate-x-20 -translate-y-20 animate-pulse`}></div>
        <div className={`absolute bottom-0 left-0 w-40 h-40 ${chatDarkMode ? 'bg-blue-700' : 'bg-blue-300'} rounded-full opacity-20 transform -translate-x-10 translate-y-10 animate-pulse`} style={{ animationDuration: '4s' }}></div>
        <div className={`absolute top-1/2 left-0 w-32 h-32 ${chatDarkMode ? 'bg-purple-700' : 'bg-purple-200'} rounded-full opacity-10 transform -translate-x-10 animate-bounce`} style={{ animationDuration: '7s' }}></div>
        <div className={`absolute bottom-1/4 right-0 w-24 h-24 ${chatDarkMode ? 'bg-blue-600' : 'bg-blue-100'} rounded-full opacity-20 transform translate-x-10 animate-bounce`} style={{ animationDuration: '5s' }}></div>
        ...
        <div className="flex flex-col md:flex-row items-center px-4 md:px-12 lg:px-20">
          ...
          <div className="flex flex-col items-center justify-center z-10 md:w-1/2 mt-8 md:mt-0">
            <div className={`relative w-full max-w-md h-[450px] ${chatDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/80 border-indigo-100'} backdrop-blur-md rounded-2xl shadow-lg p-6 border`}>
              {/* Mode and language toggle controls */}
              <div className="absolute -right-4 -top-4 flex gap-2">
                {/* Dark mode toggle */}
                <button 
                  onClick={() => setChatDarkMode(!chatDarkMode)}
                  className={`${chatDarkMode ? 'bg-gray-700 text-yellow-300' : 'bg-indigo-100 text-indigo-900'} rounded-full w-10 h-10 flex items-center justify-center shadow-md hover:scale-105 transition-transform`}
                >
                  {chatDarkMode ? <LuSun className="text-lg" /> : <LuMoon className="text-lg" />}
                </button>
                
                {/* Language toggle */}
                <button 
                  onClick={() => setIsNepali(!isNepali)}
                  className={`${chatDarkMode ? 'bg-gray-700' : 'bg-indigo-100'} rounded-full w-10 h-10 flex items-center justify-center shadow-md hover:scale-105 transition-transform`}
                >
                  <FiGlobe className={`text-lg ${chatDarkMode ? 'text-blue-400' : 'text-indigo-900'}`} />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-4 mt-6">
                {/* Bot message */}
                <div className={`self-start ${chatDarkMode ? 'bg-indigo-900/60 text-gray-100' : 'bg-indigo-100 text-gray-800'} rounded-2xl rounded-tl-none p-3 max-w-xs transition-opacity duration-500 ${animationStep >= 0 ? 'opacity-100' : 'opacity-0'}`}>
                  <p>{currentLang.greeting}</p>
                </div>
                
                {/* User message */}
                <div className={`self-end ${chatDarkMode ? 'bg-blue-900/60 text-gray-100' : 'bg-blue-100 text-gray-800'} rounded-2xl rounded-tr-none p-3 max-w-xs transition-opacity duration-500 ${animationStep >= 1 ? 'opacity-100' : 'opacity-0'}`}>
                  <p>{currentLang.userQuestion}</p>
                </div>
                
                {/* Bot response */}
                <div className={`self-start ${chatDarkMode ? 'bg-indigo-900/60 text-gray-100' : 'bg-indigo-100 text-gray-800'} rounded-2xl rounded-tl-none p-3 max-w-xs transition-opacity duration-500 ${animationStep >= 2 ? 'opacity-100' : 'opacity-0'}`}>
                  <p>{currentLang.botResponse}</p>
                </div>
                
                {/* Typing indicator */}
                <div className={`self-end ${chatDarkMode ? 'bg-blue-900/60' : 'bg-blue-100'} rounded-2xl rounded-tr-none p-3 max-w-xs flex gap-1 transition-opacity duration-500 ${animationStep === 0 ? 'opacity-100' : 'opacity-0'}`}>
                  <div className={`w-2 h-2 ${chatDarkMode ? 'bg-gray-300' : 'bg-gray-500'} rounded-full animate-bounce`}></div>
                  <div className={`w-2 h-2 ${chatDarkMode ? 'bg-gray-300' : 'bg-gray-500'} rounded-full animate-bounce`} style={{ animationDelay: '0.2s' }}></div>
                  <div className={`w-2 h-2 ${chatDarkMode ? 'bg-gray-300' : 'bg-gray-500'} rounded-full animate-bounce`} style={{ animationDelay: '0.4s' }}></div>
                </div>
              </div>
              
              {/* Chat input mockup */}
              <div className="absolute bottom-4 left-4 right-4">
                <div className={`flex items-center gap-2 ${chatDarkMode ? 'bg-gray-700' : 'bg-gray-100'} rounded-full p-2 pr-4`}>
                  <div className={`${chatDarkMode ? 'bg-gray-600' : 'bg-white'} w-8 h-8 rounded-full flex items-center justify-center`}>
                    <FiMessageCircle className="text-indigo-500" />
                  </div>
                  <div className={`${chatDarkMode ? 'text-gray-400' : 'text-gray-400'} text-sm flex-grow`}>{currentLang.inputPlaceholder}</div>
